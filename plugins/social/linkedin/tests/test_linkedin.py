"""LinkedIn provider: declared-only stub. Always unavailable, never scrapes."""
...
def test_registers_and_loads(load_provider) -> None:
    provider = load_provider("linkedin")
    assert provider.platform == "LinkedIn"


async def test_always_reports_unavailable(load_provider, monkeypatch) -> None:
    monkeypatch.delenv("LINKEDIN_ACCESS_TOKEN", raising=False)
    health = await load_provider("linkedin").health_check()
    assert health.state == "unavailable"
    assert "LINKEDIN_ACCESS_TOKEN" in health.detail
...
async def test_search_always_returns_nothing(load_provider, make_ctx) -> None:
    def handler(request):  # pragma: no cover - must never be called
        raise AssertionError("linkedin stub must never make a network request")

    assert await load_provider("linkedin").search(TARGET, make_ctx(handler)) == []


def test_declares_that_it_requires_a_key(load_provider) -> None:
    assert load_provider("linkedin").capabilities().requires_api_key is True
...
def test_provider_contains_no_scraping_path() -> None:
    ...
    forbidden = ["cookie", "captcha", "selenium", "playwright", "webdriver", "li_at"]
